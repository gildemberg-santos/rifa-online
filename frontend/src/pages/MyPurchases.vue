...
<template>
  <div class="max-w-4xl mx-auto px-4 py-8 animate-fade-in">
    ...
    <div v-else class="space-y-4">
      <div v-for="p in items" :key="p.id" class="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-sm transition-shadow">
        <div class="flex items-center justify-between mb-2">
          ...
          <span class="inline-flex px-2.5 py-1 rounded-full text-xs font-semibold" :class="statusColor(p.status)">
        ...
        <div class="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
          <div class="text-sm text-gray-500">
            ...
            <span v-if="p.paidAt" class="ml-3">Pago em {{ formatDate(p.paidAt) }}</span>
          </div>
          <p class="font-bold text-gray-900">R$ {{ (p.amount / 100).toFixed(2) }}</p>
        </div>

        <div v-if="p.type === 'RAFFLE' && p.ticketNumbers && p.ticketNumbers.length" class="mt-2">
