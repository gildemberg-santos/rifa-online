...
func main() {
	...
	raffle := &model.Raffle{
		...
		Title:       "Sorteio Teste - 10 Números",
		Description: "Rifa de teste com 10 números para validar o sorteio.",
		TicketPrice: 500,
		MaxNumbers:  10,
	...
	mockBuyers := []struct {
		...
		{"Gabriel Almeida Neto", "61932109876"},
		{"Helena Martins Duarte", "71921098765"},
		{"Igor Barbosa Correia", "81910987654"},
		{"Julia Pereira Neves", "91909876543"},
		{"Lucas Henrique Dias", "15998765432"},
	}

	rand.Seed(time.Now().UnixNano())
	rand.Shuffle(len(mockBuyers), func(i, j int) {
		mockBuyers[i], mockBuyers[j] = mockBuyers[j], mockBuyers[i]
	})
	...
	for i, t := range allTickets {
		buyer := mockBuyers[i%len(mockBuyers)]
		now := time.Now()
	...
	fmt.Println("\nDone! Raffle with 10 paid tickets created.")
}
